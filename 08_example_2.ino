...
#define _DIST_MID 200 //midum aistance to be measured (unit: mm)
#define _DIST_A 150 //midum aistance to be measured (unit: mm)
#define _DIST_B 250 //midum aistance to be measured (unit: mm)


// global variables
float timeout; // unit: us
float dist_min, dist_max, dist_raw, dist_mid , dist_a, dist_b; // unit: mm
...
// turn on the LED if the distance is between dist_min and dist_max
  ...
  else if(dist_raw == dist_mid){
    analogWrite(PIN_LED, 0);
  }
  else if(dist_a < dist_raw || dist_raw < dist_mid){
    analogWrite(PIN_LED, 127);
  }
  else if(dist_mid < dist_raw || dist_raw < dist_b){
    analogWrite(PIN_LED, 127);
  }
  else {
    analogWrite(PIN_LED, 255);
